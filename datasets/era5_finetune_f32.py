...
class era5_finetune_f32(Dataset):
    def __init__(self, data_dir='era5:s3://era5_np', split='train', **kwargs) -> None:
        super().__init__()
        # print("init begin")
    ...
    def _get_diffstd(self):
        diff_std = xr.open_dataset("./datasets/stats_diffs_stddev_by_level.nc")
        self.diff_std = []
        for vname in self.single_level_vnames:
            if vname == "u10":
                self.diff_std.append(diff_std['10m_u_component_of_wind'].data[None] / self.mean_std['std'][vname][:, 0, 0])
            elif vname == "v10":
                self.diff_std.append(diff_std['10m_v_component_of_wind'].data[None] / self.mean_std['std'][vname][:, 0, 0])
            elif vname == "t2m":
                self.diff_std.append(diff_std['2m_temperature'].data[None] / self.mean_std['std'][vname][:, 0, 0])
            elif vname == "msl":
                self.diff_std.append(diff_std['mean_sea_level_pressure'].data[None] / self.mean_std['std'][vname][:, 0, 0])
            # else:
            #     raise ValueError(f"vname {vname} not implement")
        for vname in self.multi_level_vnames:
            if vname == "z":
                self.diff_std.append(diff_std['geopotential'].sel(level=self.height_level_list).data / self.mean_std['std'][vname][self.height_level_indexes][:, 0, 0])
            elif vname == "q":
                self.diff_std.append(diff_std['specific_humidity'].sel(level=self.height_level_list).data / self.mean_std['std'][vname][self.height_level_indexes][:, 0, 0])
            elif vname == "u":
                self.diff_std.append(diff_std['u_component_of_wind'].sel(level=self.height_level_list).data / self.mean_std['std'][vname][self.height_level_indexes][:, 0, 0])
            elif vname == "v":
                self.diff_std.append(diff_std['v_component_of_wind'].sel(level=self.height_level_list).data / self.mean_std['std'][vname][self.height_level_indexes][:, 0, 0])
            elif vname == "t":
                self.diff_std.append(diff_std['temperature'].sel(level=self.height_level_list).data / self.mean_std['std'][vname][self.height_level_indexes][:, 0, 0])
            # else:
            #     raise ValueError(f"vname {vname} not implement")
        self.diff_std = np.concatenate(self.diff_std, axis=0)[:,np.newaxis,np.newaxis]
    ...
    def get_constants_data(self, constants_types):
        ...
        for vname in constants_types:
            if vname == "lsm":
                constant_data = xr.open_dataset("./datasets/lsm.nc")
                array_lst.append(constant_data['lsm'].data)
            elif vname == "orography":
                constant_data = xr.open_dataset("./datasets/height.nc")
                oro_data = constant_data['z'].data
                array_lst.append(oro_data / oro_data.max())
    ...
    def get_data(self, idxes):
        ...
        for idx in idxes:
            ...
            if self.constants_data is not None:
                return_data.append(np.concatenate((self.constants_data, b), axis=0),)
            else:
            ...
            if vname in self.single_level_vnames:
                if idx < 0:
                    ...
                    if file[17:23] == "02-29/":
                        ...
                        date_str = file[17:23]
                    url = f"{single_acc_mean_dir}/{date_str}{vname}.npy"
                    # print(url)
                else:
                    file = self.single_file_list[idx]
                ...
                if idx < 0:
                    ...
                    url = f"{multi_acc_mean_dir}/{date_str}{vname}-{height}.0.npy"
                    # print(url)
                else:
                    file = self.file_list[idx]
    ...
    def get_clim_daily(self, index):
        ...
        data_clim = self.get_data([-index])[0]
        if self.constants_data is not None:
    ...
    def __getitem__(self, index):
        ...
        array_seq = self.get_data([index + i * self.sample_stride for i in range(self.length)])
